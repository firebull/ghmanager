...
Note: timeout is in seconds. host may be ip or hostname
    ...
    def _authenticate_rcon(self):
        if not self.rconpass:
            raise RCON_Error, 'Empty RCON password.'
        if self.hl == 0:
            self.details()
        if self.hl == 1:
...
This assembles mult-packet responses and returns the raw response (sans the four \xFF's).  No parsing is done by this function.
    ...
    def details(self):
        ...
        if raw_details[0] == DETAILS_RESP_HL2:
            ...
            raise SRCDS_Error, 'Detail Query Error: Unknown response type: ' + str(raw_details)
    ...
    def _details_hl2(self, data):
        ...
        detaildict['numplayers'], data = read_byte(data)
        detaildict['maxplayers'], data = read_byte(data)
        ...
        detaildict['passworded'] = int(pworded)
    ...
    def players(self):
        ...
        if raw_players[0] != PLAYERS_RESP:
            raise SRCDS_Error, 'Player Query Error'
        data = raw_players[1:]
        playerlist = []
    ...
    def rules(self):
        ...
        if raw_rules[0] != RULES_RESP:
            raise SRCDS_Error, 'Rules Query Error'
        data = raw_rules[1:]
