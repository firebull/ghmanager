...
'''
***********************************************
Get installed maps (by files) at COD2/4 server.
Runs files_lists.py with clients rights.
...
if numrows > 0:
    ...
    if rootServer['id'] == thisServerId:
        ...
        if template['mapsPath'] != 'None':
            print '<log>Шаблон сервера: ' + template['name'] + "</log>"

            serverPath = serversPath + "/" + template['name'] + "_" + str(serverID)

            print '<log>Ищу карты по пути: ' + serverPath + '/' + template['mapsPath'] + "</log>"

            mapsPath = template['mapsPath']
            ...
            try:
                ...
                if err < 0:
                    print "<error>При попытке получения списка карт возникла ошибка: ", err, '</error>'
                elif err == 0 or err == "":
                    print "<log>Список получен</log>"

            except OSError, e:
                print "<error>При попытке получения списка карт возникла ошибка: ", e, '</error>'

        else:
            print '<error>Не указан путь поиска карт.</error>'
    else:
