...
try:
    ...
    if numrows > 0:
        log['log'] += ["INFO: Данные о сервере получены"]
        ...
        if rootServer['id'] == thisServerId:
            log['log'] += ["INFO: Сервер привязан на этот физический сервер. Продолжаю."]
            log['log'] += ["INFO: Получаю данные о пользователе"]
            ...
            log['log'] += ['INFO: Шаблон сервера: %s' % template['name']]
            ...
            try:
                ...
                for line in out.splitlines():
                    if line.strip() != '' and re.match('^(ERROR|INFO|OK)', line.strip()):
                        log['log'] += [line.strip()]

                if err < 0:
                    log['error'] += ["Не удалось установить мод/плагин, читайте подробный лог"]
                    log['log']   += ["ERROR: Не удалось запустить скрипт установки мода/плагина: %s" % err]
                elif err == 0 or err == "":
                    log['log']   += ["OK: Работа скрипта скрипта установки мода/плагина завершена успешно."]
                else:
                    log['log']   += ["ERROR: При попытке запуска скрипта установки мода/плагина возникла ошибка"]

                    for line in err.splitlines():
                        log['log'] += ["ERROR: %s" % line.strip()]

                    log['error'] += ["Не удалось установить мод/плагин, читайте подробный лог"]

            except OSError, e:
                log['error'] += ["Не удалось установить мод/плагин, читайте подробный лог"]
                log['log']   += ["ERROR: При попытке запуска скрипта установки мода/плагина возникла ошибка: %s" % e]

        else:
            log['error'] += ["Игровой сервер #%s привязан к другому физическому" % serverID]
        # Конец проверки привязки

    else:
        log['error'] += ["Нет сервера, не инициализирован или заблокирован"]
    ...
    print json.dumps(log, sort_keys=False, indent=4, separators=(',', ': '))

except Exception, e:
    log['error']  += ["При попытке запуска скрипта установки мода/плагина возникла ошибка: %s" % e]
