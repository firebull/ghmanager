#!/usr/bin/env python2
# coding: UTF-8
# Скипт предназначен для запуска/остановки/перезапуска COD 2/4 серверов

'''
***********************************************
COD2/4 Start/Stop script with clients rights.
...
serverMod = config.get('server', 'codMod')
punkbuster = config.get('server', 'punkbuster')
rconPass = config.get('server', 'rconPass')
...
pidScreenUpdateName = templateName + "-" + serverID + "-update-pb-screen.pid"
...
if action == "start" or action == 'startWithManu':
    ...
    if portStatus == False:
        ...
        if errors > 0:
            ...
            if action == 'start':
                command = start_strings.codStart(serverID,
                                                 userName,
                                                 serverIP,
                                                 serverPort,
                                                 serverSlots,
                ...
                if retcode < 0:
                    ...
                    try:
                        ...
                        if retcode < 0:
                            ...
                            '''
                            Если стартуем и ManuAdmin, то:
                            - Проверить свободен ли его порт
                            - Запустить после сервера, если тот запущен без ошибок.
                            '''
                            if startManu == True:
                                sleep(6)  # Задержка на запуск основного сервера

                                if checkOnPort(serverIP, serverPort) == True:  # Значит сервер еще не запустился
                                    print "Сервер еще не запустился, подожду еще 8 секунд"
                                    sleep(8)

                                if checkOnPort(serverIP, serverPort + 2000) == True:

                                    try:
                                        os.chdir(serverRunPath + '/mods/manuadmin')  # Переход в директорию ManuAdmin
                                        retcode = call('./startscript start', shell=True)

                                        if retcode < 0:
                                            print "Не удалось запустить скрипт ManuAdmin: ", retcode
                                            print "MANU_EXEC_STATUS:error:" + str(retcode)
                                        elif retcode == 0:
                                            print "ManuAdmin запущен"
                                            print "MANU_EXEC_STATUS:success"
                                        else:
                                            print "Не удалось запустить скрипт ManuAdmin: ", retcode
                                            print "MANU_EXEC_STATUS:error:" + str(retcode)

                                    except OSError, e:
                                        print "При попытке запуска ManuAdmin возникла ошибка:", str(e)
                                        print "MANU_EXEC_STATUS:error:" + str(e)
                                else:
                                    print "Не удалось запустить скрипт ManuAdmin: порт TCP-Admin занят"
...
#    - Если не существует, вывести сообщение
    ...
    # Сначала выключить ManuAdmin
    if os.path.exists(serverRunPath + '/mods/manuadmin'):
        if checkOnPort(serverIP, serverPort + 2000) == False:
            try:
                os.chdir(serverRunPath + '/mods/manuadmin')  # Переход в директорию ManuAdmin
                retcode = call('./startscript stop', shell=True)

                if retcode < 0:
                    print "Не удалось остановить скрипт ManuAdmin: ", retcode
                    print "MANU_EXEC_STATUS:error:" + str(retcode)
                elif retcode == 0:
                    print "ManuAdmin остановлен"
                    print "MANU_EXEC_STATUS:success"
                else:
                    print "Не удалось остановить скрипт ManuAdmin: ", retcode
                    print "MANU_EXEC_STATUS:error:" + str(retcode)
                ...
                print "При попытке остановки ManuAdmin возникла ошибка:", str(e)
                print "MANU_EXEC_STATUS:error:" + str(e)
        else:
            print "ManuAdmin не запущен"
