...
'''
Необходимо удалить старый SDK Hooks:
...
try:
    if os.path.exists(sourceModPath + '/gamedata/sdkhooks.games.txt'):
        os.remove(sourceModPath + '/gamedata/sdkhooks.games.txt')

    if os.path.exists(sourceModPath + '/extensions/sdkhooks.ext.so'):
        os.remove(sourceModPath + '/extensions/sdkhooks.ext.so')
