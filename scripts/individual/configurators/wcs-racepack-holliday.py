...
racesIni = serverPath + "/" + addonPath + "/addons/eventscripts/wcs/ini/races.ini"
racesIniToAdd = serverPath + "/" + addonPath + "/addons/eventscripts/wcs/ini/races-holliday.ini"

if os.path.exists(racesIniToAdd):
    if os.path.exists(racesIni):
        ini = open(racesIni, "a")
        newRaces = open(racesIniToAdd, "r")

        for line in newRaces:
            ini.write(line)
        ...
        os.rename(racesIniToAdd, racesIni)


text = '''
es_load HOLLIDAY_WCS // HOLIDAY Race Pack
'''
...
text = '''
sv_downloadurl "http://fdl1.teamserver.ru/fastdl/css/"
'''
