...
<div id="searchForm" style="display: none;">
		...
		<fieldset>
			<div class="control-group">
				...
				echo $form->create('Order', array('class' => 'form-inline')); 
		...
		<fieldset>
			<div class="control-group">
				...
				?>
				<div class="input-prepend input-append">
					<span class="add-on">Сервер</span><?php echo $form->input('Server.id', 
...
<?php
	...
	$effect = $js->get('#orderStatsLink')->effect('slideIn');	
	...
	$js->get('#orderStatsLink')->event('click', $event);
?>
		...
		<tr class="tbl_out" onmouseout="this.className='tbl_out'" onmouseover="this.className='tbl_hover_<?php echo $payedStatus[ $order['Order']['payed'] ]['color']   ;?>'">
			<td>
				<div class="btn-group">
					...
					$effect = $js->get('#details_'.$id)->effect('slideIn');		
					...
					$js->get('#details_'.$id)->event('click', $event);
					...
					echo $html->link('<i class="icon-trash"></i>', '#',
					  ...
					  ?>
						...
						echo $html->link('<i class="icon-ok"></i>', array('action'=>'confirm','admin',$id),
			...
			<td><?php echo $id;?></td>
			<td><?php echo $this->element('client_view', array( 'id'   => @$order['User'][0]['id'],
																'name' => @$order['User'][0]['username'])); ?>	</td>
			<td>
				<?php 
			...
			if (!empty($order['Order']['description'])){
				...
				echo $html->div('qlabs_tooltip_left qlabs_tooltip_style_1',
			...
			{
				echo $html->div('order_status', $payedStatus[ $order['Order']['payed'] ]['status']);
			}
