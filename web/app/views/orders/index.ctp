		...
		<tr>
			...
			<tr title="Информация о заказе" id="opener_<?php echo $order['id']; ?>">
				...
				<td>
					...
					$effect = $js->get('#details_'.$order['id'])->effect('slideIn');		
					...
					$js->get('#details_'.$order['id'])->event('click', $event);

					?>

				</td>
				<td><?php echo $time->niceShort($order['created']); ?></td>
				<td>
					<?php 
				...
				<td>
					<?php 
						if ($order['payed'] == 0){
							...
							$effect = $js->get('#pay_order')->effect('slideIn');		
							$event  = $js->request(array('controller'=>'Orders',
														 'action'=>'pay', $order['id']), 
							...
							$js->get('#server_cart_'.$order['id'])->event('click', $event);
						...
						else
						{
							...
							if (!empty($order['payedDate'])){								
								echo $html->tag('small', $this->Common->niceDate($order['payedDate']));								
							}							
						}
					...
					<?php 
						...
						if (!empty($order['payedBy']) && ($order['payedBy'] != 'manual' or $order['payedBy'] != 'unknown')){
							...
							if (!empty($paymentImgs[$order['payedBy']])){
								echo $html->image('icons/'.$paymentImgs[$order['payedBy']]);
							}
