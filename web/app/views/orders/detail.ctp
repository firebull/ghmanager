 ...
 */
 //pr(@$order);
 //pr(@$server);
 echo @$order['User'][0]['username']."<br/>\n";
 echo @$order['User'][0]['first_name']." ";
 echo @$order['User'][0]['second_name']."<br/><br/>\n";
...
if (!empty($order['Order']['description'])){
	...
	if($order['Order']['payed'] == 1){
		echo $html->tag('strong', 'История платежа:');		
	}
	else
	{
		echo $html->tag('strong', 'Статус оплаты:');
	}
...
?>
	...
	<?php
		...
		echo $html->tag('small', $order['Order']['description']);
