...
<br/>
<cake:nocache>
<div id="flash2"><?php echo $session->flash(); ?></div>

<?php if ($canCreateHosting === true) {?>

<?php echo $form->create('WebHosting', array('class' => 'form-inline')); ?>
...
<table border="0" cellpadding="0" cellspacing="0" width="90%" align="center">
	...
	<tr>
		<td>
			<div id="action_transparent" style="margin-top: 0px; margin-bottom:0px;">
				...
				<?php echo $form->input('User.newpasswd',array('size' => '15',
																   'type'=>'password', 
																   'id' => 'progressBar', 
																   'div' => false, 
																   'label' => false,
	...
	<tr>
		...
		<div class="control-group">
			<?php echo $form->input('domain', array( 
													 //'style' => 'width: 230px;',
													 'id' => 'domain_name',
                                                     'div' => false, 
													 'label' => false, 
			...
			<?php echo $form->input('domainList', array( 
													 'options'=>array(
...
<script type="text/javascript">
	$(function() {				
		...
		function CheckDomain(){
			...
			if ( domain.match(domain3LevelFull)) 
			{
				fullDomain = domain;
			}
			else
...
<?php 
		...
		if ($canCreateHosting === false and !empty($this->data))
		{
...
?>			
	...
	<?php
		if (is_array($this->data['Info']['email']) and !empty($this->data['Info']['email'])){
			echo $html->tag('div','email: ', array('style' => 'float: left; position: relative; '));
	...
	<?php
		if (is_array($this->data['Info']['domain']) and !empty($this->data['Info']['domain'])){
			echo $html->tag('div','Домены: ', array('style' => 'float: left; position: relative; '));
			echo '<div style="float: left; position: relative;">';
		...
		if (!empty($this->data['Info']['domain']))
		{
