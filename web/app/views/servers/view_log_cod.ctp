...
<div id="logs_types">
	<ul class="top_menu">
		...
		echo $html->link('Игровые логи сервера', '#',
		...
		$js->get('#server_game_'.$id)->event('click', $event);
		?>
		</li>
		<li class="<?php echo $class['console']; ?>">
		<?php 
		
		echo $html->link('Логи консоли сервера', '#',
		...
		$js->get('#server_console_'.$id)->event('click', $event);
		?>
		</li>
		<li class="<?php echo $class['pb']; ?>">
		<?php 
		
		echo $html->link('Логи PunkBuster', '#',
		...
		$js->get('#server_pb_'.$id)->event('click', $event);
		?>
		</li>
