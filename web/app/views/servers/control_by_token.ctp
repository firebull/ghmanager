 ...
 */
 ...
 <table>
	<tr>
		<td align="right"><?php echo $html->image('icons/servers/'.$status['gameshort'].'.png',
		...
		<div style="display: inline;" class="icons">
			<ul id="icons_<?php echo $status['id'];?>">
				...
				<li title="Принудительно запустить сервер">
							...
							echo $html->link('<i class="icon-step-forward"></i>',
											 array('controller'=>'servers',
												   'action'=>'script',
												   $status['id'],
												   'restart',
		...
		<?php
													...
													'hl2mp', 'zps')))
			{
				if (!empty($status['mapName']))
				{
					...
					echo $html->link($status['mapName'].' <i class="icon-share"></i>', '#',
					...
					$js->get('#rcon_map_'.$status['id'])->event('click', $event);
				}
			}
			else
			{
