 ...
 */
 ...
 {
 	if (!is_null($serverId) and !is_null($mapId))
 	{
		...
		if ( $canDelete === true and $installed === false)
		{
	...
	<form class="form-horizontal">
		<div class="control-group" style="float: right; width: 45%;">
			<div class="btn-group" style="float: right;">
				<?php
					echo $html->link('<i class="icon-th-list" id="list_i"></i> Списком', 
										'#',
										array( 'class' => 'btn',
											   'id' => 'list',
											   'escape' => false));
					...
					$js->get('#list')->event('click', $event);

					echo $html->link('<i class="icon-th" id="mosaic_i"></i> Мозаикой', 
										'#',
										array('class' => 'btn',
											   'id' => 'mosaic',
											   'escape' => false));
					...
					$js->get('#mosaic')->event('click', $event);
		...
		<tr>
			...
			<?php
				if (!empty($mapDesc['image'])){
					echo $html->image('gameMaps/'.$mapDesc['image'].'_thumb.jpg', array('title'  => @$mapDesc['longname'],
																						'width'  => 100,
																						'height' => 75));
				}
				else
				{
					echo $html->image('personage01_x100x75.png', array('title'  => 'Нет изображения карты',
																	   'width'  => 100,
																	   'height' => 75));
				}
			...
			<?php 
				...
				if (!empty($mapDesc['longname'])){
					...
					if (!empty($mapDesc['image'])){
						$mapImg = $html->image('gameMaps/'.$mapDesc['image'].'.jpg');
					}
					else
					{
						$mapImg = $html->image('personage01_x100x75.png', array('title' => 'Нет изображения карты',
				...
				if (strlen($mapName) <= 20){
					echo $html->tag('strong', $mapName);
				}
				else
				{
					echo $html->tag('small', $mapName);
				}
		...
		<tr>
			...
			<?php
				...
				if (!empty($mapDesc['id'])){
					...
					$link = mapLink( $id, $mapDesc['id'], @$mapDesc['official'], @$mapDesc['installed'], @$mapDesc['canDelete'], @$mapDesc['on'], $html);

					if ($link !== false)
					{
						echo $html->tag('div', $link, array('id' => 'map_action_'.$mapDesc['id']));
					}
				}
...
<script type="text/javascript">					
		...
		function mapAction ( serverId, mapId, action )
		{
			...
			$('#map_action_' + mapId).html('<?php echo $html->image('loading.gif');?> Выполняю');						    
