...
<cake:nocache>
	...
	?>		
		...
		if ( empty($installedMod) or !in_array(@$mod['Mod']['id'], @$installedMod)){
			...
			echo $html->link( $modDesc.'<nobr>'.$mod['Mod']['longname'].'&nbsp;'.$mod['Mod']['version'], '#',
			...
			$js->get('#mod_install_link_'.$mod['Mod']['id'])->event('click', $event);
		...
		<?php 
			...
			echo $html->link('Я установил по FTP', '#',
			...
			$js->get('#mod_install_link_user_'.$mod['Mod']['id'])->event('click', $event);
		...
		else if (in_array(@$mod['Mod']['id'], @$installedMod))
		{
	...
	?>		
		<td><?php 
			...
			if (@$plugin['Plugin']['installed'] ==1){
				...
				//Ссылка  для удаления плагина
				echo $html->link('<span class="ui-icon ui-icon-close" style="float: left; margin-right: 0em;"></span>', '#',
				...
				$js->get('#plugin_install_link_'.$plugin['Plugin']['id'])->event('click', $event);
	...
	<?php 
		...
		foreach ( $allTags as $tag ) {
			$tags .= $html->tag('div', $tag['name'], array(
												'id' => 'show_tag_'.$tag['id'],
												'class' => 'pluginTag',
			...
			$tags .= ', ';
			$js->get('#show_tag_'.$tag['id'])->event('click', 'highlightPlugins("'.$tag['id'].'");');
		}
		
		$tags .= $html->tag('div', 'все', array(
												'id' => 'show_tag_all',	
												'class' => 'pluginTag',						
		...
		$js->get('#show_tag_all')->event('click', '$(function() { $(".taggedPlugin").removeClass("highlight4"); $(".pluginTag").removeClass("highlight2");});');
	...
	<div class="ui-state-highlight ui-corner-all" style="margin-top: 8px; padding: 0 .7em;"> 
			...
			echo $html->link('<span>Если вы самостоятельно установили или удалили плагины через FTP, то нажмите сюда, чтобы синхронизировать 
			...
			$js->get('#plugins_resync_link')->event('click', $event);
