...
<?php
	...
	echo $text->autoLinkEmails($this->data['User']['email']);

	if (!empty($this->data['User']['email_old']))
	{
		echo '<br/>'.$text->autoLinkEmails($this->data['User']['email_old']).' (старый)';
	}
...
<?php
	...
	{
		echo $html->link('Обнулить баланс', '#',
		...
		$js->get('#user_balance_empty_'.$this->data['User']['id'])->event('click', $event);
	}
	?>
<br/>
<?php
...
echo $html->link('Изменить данные клиента', '#',
...
$js->get('#user_edit_'.$this->data['User']['id'])->event('click', $event);
?>
