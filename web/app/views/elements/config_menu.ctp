	...
	<ul>
		...
		<li id="button_config_<?php echo $config['id']; ?>"  title="<?php echo 'Просмотр конфига по пути: '.$config['path']."/".$config['name'];?>">
			...
			<?php
				...
				$loadingImage = $html->image('loading_white.gif', array('alt'=>'Loading...', 'width'=>'24', 'height'=>'24', 'style' => 'padding: 1px; padding-top: 1px;'));

				echo $html -> link( "<span class=a-menu-btn-text><strong>".$config['name']."</strong><br/>".
				...
				$effect = $js->get('#configEditor')->effect('slideIn');		
				...
				$js->get('#config_'.$config['id'])->event('click', $event);
