 ...
 */
 	...
 	{
 		$orderMessage = 'Пополнение счёта #'.$this->Common->userBill($order['User'][0]['id']).', логин: '.$order['User'][0]['username'];
 	}
...
?>
	<form name="order_<?php echo $order['Order']['id'].'_platron_'.@$using;?>" action="https://www.platron.ru/payment.php" method="POST">
		...
		<input type="hidden" name="pg_user_contact_email" value="<?php echo $order['User'][0]['email'];?>">
...
?>
	<form name="order_<?php echo $order['Order']['id'].'_'.@$using;?>" action="https://rbkmoney.ru/acceptpurchase.aspx" method="POST">
		...
		<input type="hidden" name="user_email" value="<?php echo $order['User'][0]['email'];?>">
...
?>
	<form name="order_<?php echo $order['Order']['id'].'_webmoney_'.@$using;?>" action="https://paymaster.ru/Payment/Init" method="POST" >
		...
		<input type="hidden" name="LMI_PAYER_EMAIL" value="<?php echo $order['User'][0]['email'];?>">
<?php
		...
		<div style="display: none; padding: .3em; margin-top: 5px; width: 318px;" class="flash ui-state-highlight ui-corner-all" id="qiwiForm<?php echo $formId.'_'.$order['Order']['id']?>">
			...
			<table cellpadding="0" cellspacing="0" border="0" style="margin-top: 4px; width: 316px;">
				<tr>
					<td>
						<?php 
						echo $form->input('to',array (	'size' => '25',
														'name' => 'to',
														'id'=>'qiwiPhoneNum',
														'div' => true,
...
<?php
	...
	// Форму не закрываю, т.к. отдельно будет закрытие по $form->js
