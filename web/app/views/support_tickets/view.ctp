...
$status = array(
					'read' => $html->image('icons/check_grey.gif'),
...
if( count(@$thread)>0 ){	
	...
	?>
		...
		<?php echo $text->autoLink(str_replace("\n", "<br/>", $message['text'])); ?>
		</div>
...
?>
	...
	<div class="form-actions" style="margin-bottom: 0px;">
		...
		{
		//Ссылка  для ответа
		echo $html->link('<i class="icon-comment"></i> Написать ответ','#',
		...
		$event  = $js->request(array('controller'=>'Supports',
									 'action'=>'add',$id), 
							   array('update' => '#new_message_write_'.$id,	  
									 'before'=>$loadingShow,
									 'complete'=>$loadingHide));

		$js->get('#new_message_'.$id)->event('click', $event);
	...
	?>
		...
		<?php
		echo $html->link('<i class="icon-list-alt"></i> Показать всю переписку', '#',
		...
		if ($ticketStatus != 'closed')
		{
		echo $html->link('<i class="icon-thumbs-up"></i> Закрыть тикет', array('action' => 'closeTicket', $id),
...
<?php
	...
	$js->get('#getFullThread_'.$id)->event('click', $event);				
?>
