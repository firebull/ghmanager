...
<div class="btn-toolbar">
	<div class="btn-group">
		...
		<ul class="dropdown-menu">
			<li><?php
					...
					echo $html->link('<i class="icon-tags"></i> Новый тип серверов', '#',
										array ('id'=>'type_add_new', 'escape' => false
										,'onClick'=>"$('#add_type').dialog({modal: true,position: ['center',180], show: 'clip', hide: 'clip', width: 400});"
										
										));
					$effect = $js->get('#add_type')->effect('slideIn');		
					$event  = $js->request(array('controller'=>'GameTemplates',
												 'action'=>'addType'), 
										   array('update' => '#add_type',	  
												 'before'=>$loadingShow,
												 'complete'=>$loadingHide));

					$js->get('#type_add_new')->event('click', $event);

				?>
			</li>
			<li><?php
				//Ссылка  для создания шаблона сервера
				echo $html->link('<i class="icon-tag"></i> Новый шаблон', '#',
				...
				$effect = $js->get('#add_template')->effect('slideIn');		
				$event  = $js->request(array('controller'=>'GameTemplates',
											 'action'=>'add'), 
				...
				$js->get('#template_add_new')->event('click', $event);
			...
			<li><?php
				...
				$js->get('#mod_add_new')->event('click', $event);

				?>
			</li>
			<li><?php
				//Ссылка  для создания плагина
				echo $html->link('<i class="icon-screenshot"></i> Новый плагин', '#',
				...
				$effect = $js->get('#add_plugin')->effect('slideIn');		
				$event  = $js->request(array('controller'=>'GameTemplates',
											 'action'=>'addPlugin'), 
				...
				$js->get('#plugin_add_new')->event('click', $event);
			...
			<li><?php
				...
				echo $html->link('<i class="icon-file"></i> Конфигурационный файл', '#',
				...
				$effect = $js->get('#add_config')->effect('slideIn');		
				$event  = $js->request(array('controller'=>'GameTemplates',
											 'action'=>'addConfig'), 
				...
				$js->get('#config_add_new')->event('click', $event);
	...
	<div class="btn-group">
		<?php
		//Список всех карт
		echo $html->link('<i class="icon-picture"></i> Список всех карт', '#',
		...
		$effect = $js->get('#map_list')->effect('slideIn');		
		$event  = $js->request(array('controller'=>'GameTemplates',
		...
		$js->get('#map_view_list')->event('click', $event);
		...
		if ( !empty($gameTemplates) ) {
			...
			<tr>
				...
				<div class="btn-group">
						...
						$effect = $js->get('#edit_template')->effect('slideIn');		
						$event  = $js->request(array('controller'=>'GameTemplates',
													 'action'=>'edit', $id), 
						...
						$js->get('#template_edit_'.$id)->event('click', $event);
						...
						$effect = $js->get('#link_template')->effect('slideIn');		
						$event  = $js->request(array('controller'=>'GameTemplates',
						...
						$js->get('#mod_add_'.$id)->event('click', $event);
							...
							$effect = $js->get('#link_map')->effect('slideIn');		
							$event  = $js->request(array('controller'=>'GameTemplates',
							...
							$js->get('#map_add_'.$id)->event('click', $event);

							//Иконка для добавления конфигов к шаблону
							echo $html->link('<i class="icon-list-alt"></i>', '#',
							...
							$effect = $js->get('#link_config')->effect('slideIn');		
							$event  = $js->request(array('controller'=>'GameTemplates',
							...
							$js->get('#config_add_'.$id)->event('click', $event);
	

							//Иконка для добавления услуг к шаблону
							echo $html->link('<i class="icon-bookmark"></i>', '#',
							...
							$effect = $js->get('#link_service')->effect('slideIn');		
							$event  = $js->request(array('controller'=>'GameTemplates',
							...
							$js->get('#service_add_'.$id)->event('click', $event);
						...
						//$js->get('#game_template_delete_'.$id)->event('click', $event);
						?>						
				</div>
											...
											<li>	
												<div class="ui-widget">
													<div class="ui-state-highlight ui-corner-all" style="margin-top: 0px; padding: 2px 4px 1px 3px;"> 
														...
														echo $html->link('<span class="ui-icon ui-icon-star" style="float: left; margin-right: .3em; margin-left: 0px; margin-top: 1px;"></span>'
														...
														$effect = $js->get('#link_mod')->effect('slideIn');		
														$event  = $js->request(array('controller'=>'GameTemplates',
														...
														$js->get('#'.$id.'-plugin_add_'.$mod['id'])->event('click', $event);
											...
											<li>	
												<div class="ui-widget">
													<div class="ui-state-highlight ui-corner-all" style="margin-top: 0px; padding: 2px 4px 1px 3px;"> 
														...
														$effect = $js->get('#link_plugin')->effect('slideIn');		
														$event  = $js->request(array('controller'=>'GameTemplates',
														...
														$js->get('#'.$id.'-config_add_'.$plugin['id'])->event('click', $event);
