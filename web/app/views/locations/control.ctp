...
?>
			...
			<tr style="background: #fff;">
				<td>
					<div class="btn-group">
							...
							$effect = $js->get('#edit_root_server_container')->effect('slideIn');		
							$event  = $js->request(array('controller'=>'rootServers',
														 'action'=>'edit', $id), 
							...
							echo $html->link('<i class="icon-time"></i>', '#',
							...
							$effect = $js->get('#stat')->effect('slideIn');		
							...
							$js->get('#server_stat_'.$id)->event('click', $event);
				...
				<td>  
					...
					echo $html->link( $rootServer['name'], '#',
					...
					$effect = $js->get('#ip_list_'.$id)->effect('slideIn');		
					$event  = $js->request(array('controller'=>'rootServers',
					...
					$js->get('#ip_list_link_'.$id)->event('click', $event);
	...
	<div id="link_to_location">	   
			...
			echo $html->link('+ Привязать физические серверы к локации', '#',
			...
			$effect = $js->get('#link_rootserver_to_location')->effect('slideIn');		
			$event  = $js->request(array('controller'=>'locations',
										 'action'=>'linkRootserverToLocation', $location['Location']['id']), 
								   array('update' => '#link_rootserver_to_location',	  
										 'before'=>$effect.$loadingShow,
										 'complete'=>$loadingHide));
	
			$js->get('#link_to_location_'.$location['Location']['id'])->event('click', $event);
	...
	?>
		<div id="view_stat">	   
			...
			//Ссылка  для создания физического сервера
			echo $html->link('+ Общая статистика', '#',
			...
			$js->get('#view_stat_link')->event('click', $event);
		...
		<div id="add_location_about">	   
			...
			//Ссылка  для создания физического сервера
			echo $html->link('+ Добавить новую локацию', '#',
			...
			$effect = $js->get('#add_location')->effect('slideIn');		
			$event  = $js->request(array('controller'=>'locations',
										 'action'=>'add'), 
			...
			$js->get('#location_add_new')->event('click', $event);
		...
		<div id="add_root_server_about">	   
			...
			$effect = $js->get('#add_root_server')->effect('slideIn');		
			$event  = $js->request(array('controller'=>'rootServers',
										 'action'=>'add'), 
			...
			$js->get('#root_server_add_new')->event('click', $event);
			...
			echo $html->link('+ Добавить IP', '#',
								array ('id'=>'ip_add_new', 'escape' => false
								,'onClick'=>"$('#add_ip').dialog({modal: true,position: ['center',180], show: 'clip', hide: 'clip', width: 400});"
								
								));
			$effect = $js->get('#add_ip')->effect('slideIn');		
			$event  = $js->request(array('controller'=>'rootServerIps',
										 'action'=>'add'), 
								   array('update' => '#add_ip',	  
										 'before'=>$effect.$loadingShow,
										 'complete'=>$loadingHide));
	
			$js->get('#ip_add_new')->event('click', $event);
