...
class UsersController extends AppController {
			...
			if($this->data['User']['ver_code']==$this->Session->read('ver_code'))	{
				...
				if ( $this->User->validates()) {
					...
					if ($this->User->save($this->data)){
						...
						$this->Email->subject = 'Подтверждение регистрации на портале TeamServer.RU. (Confirm Registration)';
						$this->Email->template = 'register_new_user';
						...
						$this->Session->setFlash('Подтвердите ваш e-mail, щелкнув по ссылке в письме, ' .
												 ...
												 'напишите нам на support@teamserver.ru', 'flash_login_success');
	 ...
	 */
	function rescuePass( $rescue = null ){
		if (!empty($this->data)){
			// Сначала проверить капчу
			if($this->data['User']['ver_code']==$this->Session->read('ver_code'))	{
				// Проверка логина
					...
					if ($user !== false){
						...
						if ($user['User']['email'] == $this->data['User']['email']){
							...
							if($this->User->save()){
								...
								$this->Email->from = 'TeamServer Email Robot <robot-no-answer@teamserver.ru>';
								$this->Email->replyTo = 'robot-no-answer@teamserver.ru';
								$this->Email->subject = 'TeamServer.ru: Запрос забытого пароля';
								$this->Email->template = 'request_pass';
								$this->Email->xMailer = 'TeamServer Email Robot';
								$this->Email->delivery = 'smtp';
	    						$this->Email->sendAs = 'both';
	...
	function confirmPassChange( $hash = null){
		if (!empty($hash) && $hash != 'null'){
			...
			if ($user !== false){
							...
							if($this->User->save()){
								...
								$this->Email->from = 'TeamServer Email Robot <robot-no-answer@teamserver.ru>';
								$this->Email->replyTo = 'robot-no-answer@teamserver.ru';
								$this->Email->subject = 'TeamServer.ru: Восстановление забытого пароля';
								$this->Email->template = 'resque_pass';
								$this->Email->xMailer = 'TeamServer Email Robot';
								$this->Email->delivery = 'smtp';
	    						$this->Email->sendAs = 'both';
	...
	function edit(){
		...
		if (empty($this->data))
		{
			...
			unset($this->data['User']['ftppassword']);
		...
		else
		{
			...
			if ($this->data['User']['email'] != $user['User']['email'])
			{
				...
				if (($code = $this->TeamServer->saveConfirm( 'email',
													$user['User']['id'],
												    null,
												    ...
												    )) !== false)
				{
					...
					{
						//генерация e-mail
	                    $this->Email->subject = 'Подтверждение смены email портале TeamServer.RU. (Confirm New Email)';
			...
			if (($this->data['User']['phone'] != $user['User']['phone']) and $this->data['User']['phone'] != '')
			{
							   ...
							   $this->data['User']['phone']))
				{
					...
					if ($this->data['User']['phone'] != $user['User']['phone'])
					{
						...
						if (($code = $this->TeamServer->saveConfirm( 'phone',
															$user['User']['id'],
														    null,
	...
	function confirmByCode( $action = 'check'){
		$this->layout = 'ajax';
		$this->DarkAuth->requiresAuth(array ('Admin','GameAdmin','Member'));
	...
	function delete ($id = null){
		 ...
		 */
		 ...
		 if ($id){
		 	...
		 	else if ($user['User']['last_login'] == '0000-00-00 00:00:00' or empty($user['User']['ftppassword']) )
		 	{
	...
	function confirm ($message = null) {
		$this->layout = 'ajax';
		$this->set('message', $message);

	}

	function changeFtpPass($action = null){
		$this->DarkAuth->requiresAuth();
		...
		if (empty($action)){
			 ...
			 */
			 ...
			 else if (count(@$rootServersIdsList) >= 1){
			 ...
			 else if (count(@$rootServersIdsList) <= 0){
...
//					    }
	...
	function control($sortby = 'id', $order = null) {
...
//														));
		...
		foreach ( $this->data as $key => $value ) {
				foreach($value['User'] as $userParam => $userValue){
					$this->data[$key]['User'][$userParam] = strip_tags($userValue);
				}
