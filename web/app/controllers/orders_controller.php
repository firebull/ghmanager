...
class OrdersController extends AppController {
	...
	private function checkRights ($orderId = null) {
		...
		if (@$orderId){
			...
			else if ($order['User'][0]['id'] == $sessionUserId // Да, владеет
						or
	...
	private function checkBill($userId = null){
	...
	private function updateBill($userId = null,
								$orderId = null,
		...
		if (!is_null($userId) and !is_null($sum) and !is_null($action) and !is_null($payedBy))
		{
			$this->loadModel('Bill');
			$billRecord = array();
	       			...
	       			else
	       			{
	       				...
	       				if(count($descSplitted) > 0){
		       				...
		       				foreach ( $descSplitted as $descString ) {
	       						...
	       						if (preg_match($pregWm, $descString))
	       						{
	       							$payedBy = 'webmoney';
	       							break;
	       						}
	       						else
	       						if (preg_match($pregRbk, $descString))
	       						{
	       							$payedBy = 'rbk';
	       							break;
	       						}
	...
	function control(){
		...
		if($this->Session->check('ordersChoise')){
			...
			if ( (empty($this->data['User']['username'])) && $this->Session->check('ordersChoise.username') )
				{
				 	$this->data['User']['username'] = $this->Session->read('ordersChoise.username');
				}
			// Проверка ID сервера
			if ( (empty($this->data['Server']['id'])) && $this->Session->check('ordersChoise.serverId') )
				{
				 	$this->data['Server']['id'] = $this->Session->read('ordersChoise.serverId');
				}
		}
		...
		if (!empty($this->data['User']['username']) and $this->data['User']['username'] != 'all'){
			...
			$this->Order->User->bindModel(array(
													'hasAndBelongsToMany' => array(
			...
			$searchUser = $this->Order->User->findByUsername($this->data['User']['username']);
			...
			if (!empty($searchUser)){
				...
				$this->Session->write('ordersChoise.username', $this->data['User']['username']);
		...
		if (!empty($this->data['Server']['id']) and $this->data['Server']['id'] != 'all'){
			...
			$searchServer = $this->Order->Server->findById($this->data['Server']['id']);
			...
			if (!empty($searchServer)){
				...
				$this->Session->write('ordersChoise.serverId', $this->data['Server']['id']);
	...
	function add($type = null, $game = null) {
		/*
		...
		if (!empty($this->data)) {
			...
			if ($this->data['Server']['privateType'] == 2){ // приватный с автоотключением
			...
			if (!@$unAvailiableService and $correctSum > 0 and $month > 0)	{ // Если услуги учтены верно, формируем и сохраняем заказ дальше
				...
				if ($template['GameTemplate']['name'] == 'l4d2-t100')
				{
				...
				*/
				...
				$this->data['Service']['Service'] = @$serviceList;
				...
				if ($this->ServerComp->save($server)){
					...
					if ($payFull === true)
					{
						...
						if ($this->Order->save($order))
						{
							...
							$this->OrderClean->id = $this->Order->id;
							unset($order);
							$this->OrderClean->bindModel(array(
												'hasAndBelongsToMany' => array(
																				'Server' => array()
																			)));
							$order = $this->OrderClean->read();
							...
							$this->redirect(array('action' => 'detail', $order['OrderClean']['id']));
							return true;
						}
						else
						{
		...
		else
		{
			...
			if (!empty($locationsList))
			{
				...
				//$this->data['GameTemplate']['id']=$this->data['GameTemplate'][0]['id'];
	...
	function prolongate($serverId = null) {
		...
		if ($serverId){
			...
			if (!empty($server)){
					...
					)
				{
					...
					else if ($this->data and $server['ServerTemplateUser']['initialised'] == 1)
					// Запрос на формирование заказа на продление
					{
						if (
							...
							if ($server['ServerTemplateUser']['privateType'] == 2){ // приватный с автоотключением
							...
							if ($this->data['Order']['payFrom'] == 'part')
							{
								...
								if ($partAmount > 0 && $balance >= $partAmount && $partAmount < $sum)
								{
									// Частичная оплата
									$order['Order']['description'] .= 'Для оплаты заказа с Личного счёта будет списано '.$partAmount.' руб. после поступления средств.<br/>';
								}
							...
							if ($payFull === true)
							{
								...
								if ($this->Order->save($order))
								{
									...
									$this->redirect(array('action' => 'detail', $this->OrderClean->id));
								}
								else
								{
									$this->Session->setFlash('Возникла ошибка при сохранении заказа в системе: '.mysql_error(), 'flash_error');
	...
	function makeDeposit($sum = null){
		$this->DarkAuth->requiresAuth();

		if (is_null($sum))
		{
			$sum = $this->data['Order']['amount'];
		}
		...
		if ($sum > 0)
		{
			...
			$order['OrderClean']['sum'] = $sum;
			$order['OrderClean']['sumToPay'] = $sum;
			$order['OrderClean']['description'] = 'Пополнение лицевого счёта';
			$order['User']['id'] = $userId;

			if ($this->OrderClean->save($order))
			{
				$confirmMessage =   "Заказ на сумму <strong>".$order['OrderClean']['sum'].
							  		" руб.</strong> на пополнение лицевого счёта #".$userAcc." сформирован.<br/>\n" .
							 		"Выберите способ оплаты.";

				$this->Session->setFlash($confirmMessage, 'flash_success');
				$this->set('userAcc', $userAcc);
				$this->redirect(array('action' => 'pay', $this->OrderClean->id));
			}
			else
			{
	...
	function createPurchase($system = null, $using = null, $orderId = null){
		/*
		...
		if ($this->checkRights(@$orderId)){
			...
			if ($order['Order']['payed'] != 1 && !empty($paymentParams)){
				...
				else if ($system == 'yandex'){ // Yandex Money

				}
				else if ($system == 'bankCard'){ // Visa/MasterCard
	...
	function confirm($confirmer = null, $id = null){
		$this->loadModel('OrderClean');
		$this->loadModel('ServerRootserver');
		...
		// По большому счёту - это костыль после ввода нового биллинга =(
		$this->OrderClean->bindModel(array(
											'hasAndBelongsToMany' => array(
																			'Server' => array()
																		)));
		...
		// Административное подтверждение оплаты
			...
			$this->OrderClean->id = $id;
			$order = $this->OrderClean->read();
			$order['OrderClean']['payed'] = 1;
			$order['OrderClean']['payedBy'] = 'manual';
		}
			...
			if (!empty($this->params['form']['pg_xml'])){
				if (!empty($paymentParams['platron']))
				{
					...
					if ($hash == $request['Request']['pg_sig'])
					{
						...
						$this->OrderClean->id = $request['Request']['pg_order_id'];
						$order = $this->OrderClean->read();
						// Проверить, не оплачен ли уже заказ
						if (!empty($order['OrderClean']) && $order['OrderClean']['payed'] == 1)
						{
						...
						if (!empty($order['OrderClean']) && $order['OrderClean']['payed'] == 0)
						{
							...
							if (floatval($order['OrderClean']['sumToPay']) <= floatval($request['Request']['pg_amount'])
									and $request['Request']['pg_currency'] == 'RUR')
							{
								...
								if (!empty($request['Request']['pg_result']) and !empty($request['Request']['pg_payment_date']))
								{
									...
									{
										$order['OrderClean']['sumPayed'] = $request['Request']['pg_amount'];
										$order['OrderClean']['sumGot'] = $request['Request']['pg_net_amount'];

										// Тестовый платеж не проводим. Только сообщение пишем
										if (!in_array($request['Request']['pg_payment_system'], array('TEST', 'TESTCARD')))
										{
											$order['OrderClean']['description'] = @$order['OrderClean']['description'].
											'<br/>'.
										...
										$this->set('status', 'ok');
										$order['OrderClean']['payedBy'] = 'platron';
									}
								...
								else
								{
									...
									{
										$order['OrderClean']['description'] .= '<br/>Счёт планируется к оплате через '.$platronSystem;
									}
			...
			if (!empty($this->params['form'])){
				...
				if (!empty($paymentParams) && // удалось считать ini с параметрами
					...
					// 3) Теперь проверить сумму заказа
						$this->OrderClean->id = $rbkMsg['orderId'];
						$order = $this->OrderClean->read();
						// Проверить, не оплачен ли уже заказ
						if (!empty($order['OrderClean']) && $order['OrderClean']['payed'] == 1){
						...
						else if ( !empty($order['OrderClean']) && $order['OrderClean']['sumToPay'] == $rbkMsg['recipientAmount']){
							// Все, теперь формируем записи в базу и сохраняем статус заказа
							$id = $rbkMsg['orderId'];
							if ($rbkMsg['paymentStatus'] == 5) { // Заказ оплачен
								$order['OrderClean']['sumPayed'] = $rbkMsg['recipientAmount'];
								$order['OrderClean']['sumGot'] = $rbkMsg['recipientAmount']*(1-($paymentParams['rbk']['procent']/100));
								$order['OrderClean']['description'] = @$order['OrderClean']['description'].
								'<br/>'.
								...
								$order['OrderClean']['payed'] = 1;
								$order['OrderClean']['payedBy'] = 'rbk';
							}
							else if ($rbkMsg['paymentStatus'] == 3){ // Заказ принят на обработку
								 $order['OrderClean']['description'] = @$order['OrderClean']['description'].
				...
				if ( $paymentParams['yandex']['ShopID'] == @$yandexMsg['shopId'] // ID продавца, т.е. наш, верен
					...
					*/
					...
					if ($hash == $yandexMsg['md5']){ // Хэш верен
						...
						$this->OrderClean->id = $yandexMsg['customerNumber'];
						$order = $this->OrderClean->read();
						if (!empty($order)){
							if ($order['OrderClean']['sumToPay'] == $yandexMsg['orderSumAmount']){ // Сумма верна
								// Все, теперь формируем записи в базу и сохраняем статус заказа
								...
								else if ( $yandexMsg['action'] == 'PaymentSuccess'
										     and
										  $yandexMsg['orderIsPaid'] == 1
										 )
								{ // Уведомление об оплате

									$order['OrderClean']['sumPayed'] = $yandexMsg['orderSumAmount'];
									$order['OrderClean']['sumGot'] = $yandexMsg['shopSumAmount'];
									$order['OrderClean']['description'] = @$order['OrderClean']['description'].
								    '<br/>'.
									'Оплачен через Яндекс.Деньги:<br/>' .
									'Платёж Yandex #'.$yandexMsg['invoiceId'].' осуществлён '.$yandexMsg['paymentDateTime'];
									$order['OrderClean']['payed'] = 1;
									$order['OrderClean']['payedBy'] = 'yandex';
								}
								else
								{
									$this->set('errno',200);
		...
		/********************************************************************************************/
		/********************************************************************************************/
		else if ($confirmer == 'yamoney'){ // (напрямую на кошелёк)
			//pr($this->params['form']);
				...
				$paymentParams = parse_ini_file("../config/payment_params.ini.php", true);
					...
					*/
					...
					if ($hash == $yandexMsg['sha1_hash']){ // Хэш верен
						...
						$this->OrderClean->id = $yandexMsg['label'];
						$order = $this->OrderClean->read();
						if (!empty($order)){
							// Сумма верна: больше или равна сумме счёта
							if (round(floatval($order['OrderClean']['sumToPay'])*0.995, 2) <= floatval($yandexMsg['amount'])){
								// Все, теперь формируем записи в базу и сохраняем статус заказа
								$id = $yandexMsg['label'];
								if ( $order['OrderClean']['payed'] == 1 )	// Если приходят уведомления, но заказ оплачен, надо сообщить об этом Яндексу
								{
									$this->set('status','OK');
									return true;
								}
								else
								if ($yandexMsg['test_notification'] == 'true')
								{ // Уведомление об оплате

									$order['OrderClean']['sumPayed'] = $yandexMsg['amount'];
									$order['OrderClean']['sumGot'] = 0;
									$order['OrderClean']['description'] = @$order['OrderClean']['description'].
								    '<br/>'.
								...
								{ // Уведомление об оплате
									...
									$order['OrderClean']['sumPayed'] = round(floatval($yandexMsg['amount'])/0.995, 2);
									$order['OrderClean']['sumGot'] = $yandexMsg['amount'];
									$order['OrderClean']['description'] = @$order['OrderClean']['description'].
								    '<br/>'.
									'Оплачен через Яндекс.Деньги:<br/>' .
		...
		else if ($confirmer == 'webmoney')
		{
			...
			if (!empty($this->params['form']) && !empty($paymentParams)){// удалось считать ini с параметрами
				...
				if ($paymentParams['wm']['wallet'] == $wmMsg['LMI_MERCHANT_ID']){
					// 2) Проверить соответсвие заказа запросу
						$this->OrderClean->id = $wmMsg['LMI_PAYMENT_NO'];
						$order = $this->OrderClean->read();
						if (!empty($order)){
							if ($order['OrderClean']['sumToPay'] == $wmMsg['LMI_PAYMENT_AMOUNT']){ // Сумма верна
								...
								if ($order['OrderClean']['payed'] == 1){ // Заказ может быть уже оплачен
									$this->set('status','YES');
									$this->render();
									return true;
								}
								else if (!empty($wmMsg['LMI_PREREQUEST']) && $wmMsg['LMI_PREREQUEST'] == 1){
								...
								else
								{
									...
									*/
									...
									if ($hash == $wmMsg['LMI_HASH']){ // Хэш верен
										...
										$id = $wmMsg['LMI_PAYMENT_NO'];
										$order['OrderClean']['sumPayed'] = $wmMsg['LMI_PAYMENT_AMOUNT'];
										$order['OrderClean']['sumGot'] = $wmMsg['LMI_PAYMENT_AMOUNT']*(1-($paymentParams['wm']['procent']/100));
										$order['OrderClean']['description'] = @$order['OrderClean']['description'].
								        '<br/>'.
										...
										$order['OrderClean']['payed'] = 1;
										$order['OrderClean']['payedBy'] = 'webmoney';
							...
							{
								$this->set('message','Неверная сумма заказа, должна быть '.$order['OrderClean']['sumToPay'].' руб.');
							}
					}
		...
		else if ($confirmer == 'qiwi')
		{
			...
			if (@$soap->load('php://input') and !empty($paymentParams))
			{
				...
				if (!empty($xmlAsArray['Envelope']['Body']['UpdateBill']))
				{
					...
					{
						// 3) Проверить соответсвие заказа запросу
						$this->OrderClean->id = $qiwiReq['txn'];
						$order = $this->OrderClean->read();
						if (!empty($order) and $order['OrderClean']['payed'] == 0)
						{
							$id = $qiwiReq['txn'];
							...
							if ($qiwiStatus > 0 and $qiwiStatus < 50)
							{
								...
								$order['OrderClean']['description'] = @$order['OrderClean']['description'].'<br/>'.'Счёт выставлен в системе Qiwi, но еще не оплачен. <br/>Статус: #'.$qiwiStatus;
								$status = 0;
							}
							else
							if ($qiwiStatus >= 50 and $qiwiStatus < 60)
							{
								// счет в процессе проведения
								$order['OrderClean']['description'] = @$order['OrderClean']['description'].'<br/>'.'Счёт условно оплачен и проводится в системе Qiwi, но подтверждение еще не получено. <br/>Статус: #'.$qiwiStatus;
								$status = 0;
							}
							else
							if ($qiwiStatus == 60)
							{
								...
								$order['OrderClean']['sumPayed'] = $order['OrderClean']['sumToPay'];
								$order['OrderClean']['sumGot'] = $order['OrderClean']['sumToPay']*(1-($paymentParams['qiwi']['procent']/100));
								$order['OrderClean']['description'] = @$order['OrderClean']['description'].
						        '<br/>'.
								'Оплачен через Qiwi:<br/>' .
								'Платеж подтверждён '.date('H:i:s d.m.Y', time()).'.';
								$order['OrderClean']['payed'] = 1;
								$order['OrderClean']['payedBy'] = 'qiwi';
							}
							else
							if ($qiwiStatus > 100)
							{
								...
								*/
								...
								$order['OrderClean']['description'] = @$order['OrderClean']['description'].'<br/>'.'Счёт был выставлен в системе Qiwi, но отменён. <br/>Статус: '.$qiwiStatusText;
						...
						if (!empty($order) and $order['OrderClean']['payed'] == 1)
						{
	...
	// Проводим заказ
	private function orderTransaction ($order = array(), $confirmer = null){
			...
			if (!empty($order))
			{
				...
				$id = $order['OrderClean']['id'];

				$order['OrderClean']['payedDate'] = date('Y-m-d H:i:s', time());

				$order['User']['id'] = $order['User'][0]['id'];
				...
				else
				{
					...
					if (@$order['OrderClean']['payed'] == 1 and $confirmer == 'admin'){
						$order['OrderClean']['sumPayed'] = $order['OrderClean']['sum'];
						// Указать примерную сумму зачисления
						// TODO: Указывать реальную сумму зачисления в форме подтверждения админом.
						$order['OrderClean']['sumGot'] = round($order['OrderClean']['sum']*0.96, 2);
					}

				}

				if ($this->OrderClean->save($order, $validate)){

					if ($order['OrderClean']['payed'] == 1){ // Если заказ оплачен
						// Внести оплаченную сумму на счёт -> $order['OrderClean']['sumPayed']
						if ($order['OrderClean']['payedBy'] == 'manual')
						{
							...
							else
							{
								...
								$order['OrderClean']['sumGot'] = $order['OrderClean']['sum'];
							}
						...
						if ($order['OrderClean']['payedBy'] != 'internal'  // Не пополнять счёт при оплате с Личного счёта
								or
							empty($order['Server']) // Пополнять, если заказ не привязан к серверу
							)
						{
							if ($this->updateBill(	$order['User'][0]['id'],
													$id,
													$order['OrderClean']['sumPayed'],
													$order['OrderClean']['sumGot'],
													'put',
													$billDescPut,
													$order['OrderClean']['payedBy']
												   )
								)
								{
									...
									if (!empty($order['Server']))
									{
										$orderDesc = $order['OrderClean']['description'].
			...
			//															));
						...
						$billAmount = $this->checkBill($order['User'][0]['id']);
	...
	function cancel($id = null){
		...
		if ($order['Order']['payed'] == 1){
			...
			if ($this->Order->delete($id)){
				...
				if ($order['Order']['payedBy'] == 'manual')
				{
					...
					$this->checkBill($order['User'][0]['id']);
				}
			}
			else
			{
		...
		else if ( $this->checkRights($id) and $order['Order']['payed'] == 0 )
		{
	...
	function clearExpired(){
		...
		$orders = $this->Order->find('all', array( 'conditions' => array( 'payed' => 0,
																		  'created <' => $dateOffset) ));
		...
		if(!$this->Order->deleteAll(array( 'id' => $ordersList), false))
		{
			$error = '<br/>Не удалось удалить заказы: '.mysql_error();
		}

		// Теперь удалить серверы
		if (!$this->Order->Server->deleteAll(array( 'id' => $serversList), false))
		{
	...
	function balanceEmpty($userId = null){
		$this->layout = 'ajax';
		$this->DarkAuth->requiresAuth(array('Admin','OrdersAdmin'));
