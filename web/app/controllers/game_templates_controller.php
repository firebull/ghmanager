...
class GameTemplatesController extends AppController {
	...
	function control() {
		...
		$this->data = $this->GameTemplate->find('all');
	...
	function edit ($id = null){
		...
		if (empty($this->data)) {
			...
			$this->data = $this->GameTemplate->read();
			$typesList = $this->Type->find('list',array('fields' => array('id','longname')));
			asort($typesList);
			$this->set('typesList', $typesList);
	...
	function addMap() {
		...
		if ($this->data){
			...
			if ( empty($this->data['Map']['map_image']['tmp_name'])
					or
				(!is_null($this->data['Map']['map_image']['tmp_name']) and $this->uploadMapImage())
	...
	function editMap ( $id = null) {
		...
		if ($this->data){
			...
			if ($this->Map->save($this->data)) {
			    ...
			    if (!is_null($this->data['Map']['map_image']['tmp_name']) and $this->uploadMapImage()){
			    	// Переименовать картинку под ID карты
			    	$imagesPath = WWW_ROOT."/img/gameMaps/";
	...
	function commonMapList () {
		$this->DarkAuth->requiresAuth(array('Admin'));
		$this->loadModel('Map');

		$maps = $this->Map->find('all');
		$this->data['MapList']['none'] = array();
		foreach ( $maps as $map ) {
			...
			if (!empty($map['GameTemplate'][0]['name'])){
				$this->data['MapList'][$map['GameTemplate'][0]['name']][] = $map;
			}
			else
			{
				$this->data['MapList']['none'][] = $map;
			}
	...
	function linkMapToTemplate($id = null) {
		$this->DarkAuth->requiresAuth(array('Admin'));
		$this->loadModel('Map');
		...
		else
		{
				...
				$this->data = $this->GameTemplateMap->read();
	...
	function linkModToTemplate($id = null) {
		$this->DarkAuth->requiresAuth(array('Admin'));
		$this->loadModel('Mod');
	...
	function linkPluginToTemplate($id = null) {
		$this->DarkAuth->requiresAuth(array('Admin'));
		$this->loadModel('Mod');
		$this->loadModel('Plugin');
	...
	function linkConfigToTemplate($id = null) {
		$this->DarkAuth->requiresAuth(array('Admin'));
		$this->loadModel('Config');
	...
	function linkServiceToTemplate($id = null) {
		$this->DarkAuth->requiresAuth(array('Admin'));
	...
	function linkPluginToMod($id = null) {
		$this->DarkAuth->requiresAuth(array('Admin'));
		$this->loadModel('Mod');
		$this->loadModel('Plugin');
		$this->loadModel('Config');
	...
	function linkConfigAndTagToPlugin($id = null) {
		$this->DarkAuth->requiresAuth(array('Admin'));
		$this->loadModel('Plugin');
	...
	function getMods( $id = null) {
			...
			$this->GameTemplate->id = $id;
			$this->data = $this->GameTemplate->read(array('id','longname'));
			//********************************************************
		...
		function getTemplates( $id = null) {
				...
				$this->data = $this->GameTemplate->Type->read( array( 'id','longname' ));
