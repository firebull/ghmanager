...
<div id="logs_types">
	<ul class="top_menu">
		<li class="<?php echo $class['startup']; ?>">
		<?php

		echo $this->Html->link('Логи запуска сервера', '#',
						  array ('id'=>'server_startlog_'.$id, 'escape' => false)
						  );

		$event  = $this->Js->request(array('controller'=>'servers',
									 'action'=>'viewLog', $id, "startup"),
							   array('update' => '#server_log_view_'.$id,
									 'before'=>$loadingShow,
									 'complete'=>$loadingHide
									 ));

		$this->Js->get('#server_startlog_'.$id)->event('click', $event);
		?>
		</li>
		...
		$this->Js->get('#server_runlog_'.$id)->event('click', $event);
		?>
		</li>
		...
		$this->Js->get('#server_updatelog_'.$id)->event('click', $event);
		?>
		</li>
