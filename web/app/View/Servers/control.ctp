...
<div id="searchButton" style="margin-bottom: 5px;">
	<?php
			...
			if (@$locationId !== null and @$locationId != 'all') {
				$searchParam = 'Локация: '.@$locationsList[$locationId].'; ';
				$isSearch = true;
			}

			if ($statusChoise !== null and @$statusChoise != 'all') {
				$searchParam .= 'Статус: '.@$statusList[$statusChoise].'; ';
				$isSearch = true;
			}

			if (@$searchUserName !== null and @$searchUserName != 'all') {
				$searchParam .= 'Клиент: '.@$searchUserName.'; ';
				$isSearch = true;
			}

			if (@$searchServerId !== null and @$searchServerId != 'all') {
				$searchParam .= 'ID сервера: #'.@$searchServerId.'; ';
				$isSearch = true;
			}

			if (@$searchServerIp !== null and @$searchServerIp != 'all') {
				$searchParam .= 'IP сервера: '.@$searchServerIp;
				$isSearch = true;
			}

			if (@$searchServerPort !== null and @$searchServerPort != 'all') {
...
<div id="searchForm" style="display: none;">
	...
	echo $this->Form->create('Server', array('class' => 'form-inline'));
	echo $this->Form->input('User.username', array(	'type' => 'hidden',
												'value' => 'all'));
	echo $this->Form->input('Server.id', array(
										'type'=>'hidden',
										'value' => 'all'));
	echo $this->Form->input('Server.address', array(
										'type'=>'hidden',
										'value' => 'all'));
	echo $this->Form->input('Server.port', array(
										'type'=>'hidden',
										'value' => 'all'));
	?>
	<fieldset>
		<div class="control-group">
			<div class="input-prepend">
				<span class="add-on">Локация</span><?php 	echo $this->Form->input('Location.id', array('options' => @$locationsList,
										'selected'=>@$locationId,
										'div' => false,
										'id'  => 'locations',
										'label' => false)); ?>
			...
			<div class="input-prepend">
				<span class="add-on">Статус</span><?php 	echo $this->Form->input('Server.status', array('options' => $statusList,
										'selected'=> @$statusChoise,
										'div' => false,
										'id'  => 'statuses',
										'label' => false)); ?>
			</div>
			<?php echo $this->Form->submit('Отобрать', array('class' => 'btn btn-primary', 'div' => false));
			?>
	...
	echo $this->Form->create('Server', array('class' => 'form-inline'));
	echo $this->Form->input('Location.id', array(
										'type'=>'hidden',
										'value' => 'all'));
	echo $this->Form->input('Server.status', array(
										'type'=>'hidden',
										'value' => 'all'));
	echo $this->Form->input('Server.id', array(
										'type'=>'hidden',
	...
	<fieldset>
		<div class="control-group">
			<div class="input-prepend input-append">
				<span class="add-on">&nbsp;&nbsp;&nbsp;ID #</span><?php echo $this->Form->input('Server.id',
		...
		<div class="control-group">
			...
			<div class="input-prepend">
				<span class="add-on">IP</span><?php
					...
					echo $this->Form->input('Server.address',
			...
			<div class="input-prepend">
				<span class="add-on">Порт</span><?php echo $this->Form->input('Server.port',
			...
			<?php echo $this->Form->submit('Искать', array('class' => 'btn btn-primary', 'div' => false));?>
...
<table class="intext" border="0" cellpadding="0" cellspacing="0">
	<tr>
		...
		<th width="280"></th>
		<th>Слотов</th>
		<th><?php echo $this->Paginator->sort('Адрес', 'address'); ?>/Cервер</th>
			...
			<td><?php
				...
				if (!empty($server['Server']['giftDays'])
					...
					strtotime($server['Server']['giftExpires']) > time()) {
						...
						echo $this->Html->tag( 'small', 'Из них '.$server['Server']['giftDays']." в подарок");
	...
	<div id="add_server_about">
		...
		$this->Js->get('#server_add_new')->event('click', $event);
