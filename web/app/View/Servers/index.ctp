...
<div id="servers_list">
	...
	echo $this->Html->image('loading.gif', array('alt' => 'Загрузка новостей...',
										   'width'  =>'16',
                                           'height' => '16',
                                           'style'  => 'margin: 10px;'));
	?>
			...
			<table class="order">
				...
				<tr>
					<td><?php
							...
							echo $this->Html->link($this->Html->image('bigicons/ico-game.png',
							...
							$effect = $this->Js->get('#add_server')->effect('slideIn');
							...
							$this->Js->get('#game_server_add_new')->event('click', $event);
					...
					<td><?php
							...
							echo $this->Html->link($this->Html->image('bigicons/ico-voice.png',
							...
							$this->Js->get('#voice_server_add_new')->event('click', $event);
					...
					<?php
							...
							echo $this->Html->link($this->Html->image('bigicons/ico-eac.png',
							...
							$this->Js->get('#eac_server_add_new')->event('click', $event);
...
<?php $this->Js->get('#news_opener')->event('click', 'newsShow();'); ?>
<div id="news"></div>
<script type="text/javascript">
<?php
	$newsInitLoader  = $this->Js->request(array('controller' => 'Messages',
												 'action'=>'init'),
										   array('update' => '#news'));
	$newsLoader      = $this->Js->request(array('controller' => 'Messages',
												 'action'=>'index'),
										   array('update' => '#news'));
	echo $newsInitLoader;
?>
...
if ( !empty($serversGrouped) ) { // Если есть список
	...
	//**********************************************************************************
		...
		<tr id="opener_<?php echo $server['Server']['id']; ?>" class="tbl_out_hide" onmouseout="this.className='tbl_out_hide'" onmouseover="this.className='tbl_hover_red'">
			...
			<?php
			} elseif (($now < $this->Time->fromString($server['Server']['payedTill'])) && ($server['Server']['initialised'] == 1 ))
		...
		<tr class="tbl_out" id="server_string_<?php echo $server['Server']['id']; ?>">
			...
			<td class="left">
				<div class="cuttext" style="max-width: 125px;">
					<?php
						...
						echo $this->Html->link($serverName, '#',
						...
						$effect = $this->Js->get('#name_server')->effect('slideIn');
						...
						$this->Js->get('#server_name_'.$server['Server']['id'])->event('click', $event);
			...
			<td><?php
				...
				} elseif ($server['Server']['scaleTime'] >= 0 and $server['Server']['scaleTime'] <= 0.15) {
					$scaleColor = '#970405';
					$dateColor = '#DD1500';

					if (($this->Time->toUnix($server['Server']['payedTill']) - time()) < 86400) {
				...
				echo $this->Html->tag('small', $this->Common->niceDate($server['Server']['payedTill']),
				...
				if (!empty($server['Server']['giftDays'])
					...
					strtotime($server['Server']['giftExpires']) > time()) {
						...
						echo $this->Html->link( 'Из них '.$server['Server']['giftDays']." в подарок!", 'http://forum.teamserver.ru/index.php?topic=75.0',
			...
			<?php
				...
				echo $this->Html->tag('div', '', array( 'style' => 'background-color: '.$scaleColor.'; ' .
		...
		<tr id="opener_<?php echo $server['Server']['id']; ?>">
			...
			<?php
			} elseif (($now < $this->Time->fromString($server['Server']['payedTill'])) && ($server['Server']['initialised'] == 0 ))
...
?>
		<tr  <?php
			...
			} elseif ($journalAction['status'] == 'warn') {
		...
		?>>
			<td>
				<?php
					...
					$this->Format= '#%1$08d';
					printf($this->Format, $journalAction['id']);

				?>
			</td>
			<td style="text-align: left;"><?php echo $journalAction['action']; ?></td>
			<td><?php echo $this->Html->tag('small', @$userNice[$journalAction['creator']]); ?></td>
			<td>
				<?php
...
<script type="text/javascript" language="javascript">
		...
		function newsShow() {
			$('#news').html('<?php echo $this->Html->image('loading.gif', array('alt'=>'Загрузка новостей...', 'width'=>'16', 'height'=>'16')); ?>');
			<?php echo $newsLoader; ?>
...
<script type="text/javascript">
	...
	function setButtonEac(serverId) {
		...
		} else if ( $('#eac_switch_button_' + serverId).hasClass('ui-icon-play')) {
