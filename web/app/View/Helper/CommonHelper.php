...
class CommonHelper extends AppHelper {
	...
	public function niceDate($date, $format = 'full', $type = 'text') {
	...
	public function niceDateDiff($dateFrom = null, $dateTo = null) {
		...
		if ($uDateDiff <= 120) {
			return 'Только что';
		} elseif ($uDateDiff > 120 and $uDateDiff <= 300) {
			return 'Менее 5 минут';
		} elseif ($uDateDiff > 300 and $uDateDiff <= 1800) {
			return 'Менее часа';
		} elseif ($uDateDiff > 1800 and $uDateDiff <= 86400) {
			return 'Больше одного часа';
		} elseif ($uDateDiff > 86400 and $uDateDiff <= 604800) {
			return 'Больше суток';
		} elseif ($uDateDiff > 604800 and $uDateDiff <= 2592000) {
			return 'Больше недели';
		} elseif ($uDateDiff > 2592000) {
	...
	public function getLoadIndicator($cur = null, $max = null) {
		if ($max > 0) {
			$load = ($cur/$max)*100;
		} else {
	...
	public function versionStatus($current = null, $userVersion = null, $type = null) {
		if (@$type === 'hl1') {
			$version = preg_split('/\//', $userVersion);
			$userVersion = $version[0];
		}

		if (!($current === null or $userVersion === null)) {
	...
	public function genSalt($length = 16) {
	...
	public function parsePhoneNum($num = null) {
		if ($num !== null) {
