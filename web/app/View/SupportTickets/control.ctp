...
<div id="searchForm" style="display: none;">
		<fieldset>
			<div class="control-group">
				...
				echo $this->Form->create('SupportTicket', array('class' => 'form-inline'));
		...
		<tr>
			...
			<tr <?php echo 'style="'.$style.'"';?> >
				...
				<td class="left"  style="cursor:pointer;" title="Раскрыть/обновить ветку" id="opener_<?php echo $ticket['SupportTicket']['id']; ?>">
					...
					<?php
		                $effect = $this->Js->get('#ticket_more_'.$ticket['SupportTicket']['id'])->effect('slideIn');
						...
						$this->Js->get('#opener_'.$ticket['SupportTicket']['id'])->event('click', $event);
					?>
				</td>
				...
				<td style="text-align: left;"><?php
							if ( !empty($ticketStates[$ticket['SupportTicket']['id']]))
							{
								...
								$timeText = $this->Common->niceDateDiff($ticketStates[$ticket['SupportTicket']['id']]['created'], date("d.m.Y H:i:s"));

								if ($ticketStates[$ticket['SupportTicket']['id']]['answerBy'] == 'owner')
								{
									echo $this->Html->tag('span', $timeText.'!', array('class' => 'highlight2'));
								}
								else
								if
								( empty($ticketStates[$ticket['SupportTicket']['id']]['answerBy']) or $ticketStates[$ticket['SupportTicket']['id']]['answerBy'] == 'support')
								{
									echo $this->Common->niceDateDiff($ticketStates[$ticket['SupportTicket']['id']]['created'], date("d.m.Y H:i:s"));
								}
							}
							else
							{
								echo $this->Html->tag('nobr', $this->Common->niceDate($ticket['SupportTicket']['created']));
							}
							 ?>
				</td>
			</tr>
...
{
	echo $this->Html->tag('h3', 'Еще нет ниодного тикета.');
}
?>
<center>
