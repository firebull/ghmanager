...
class OrdersController extends AppController {
    ...
    public function add($type = null, $game = null) {
        ...
        if (!empty($this->data)) {
            ...
            $locationsList = $this->Location->find('all', ['fields'     => ['id','name', 'collocation'],
                                                           'conditions' => ['enabled'=>'1']]);
            if (!empty($locationsList)) {
                asort($locationsList);
                $this->set('locationsList', Hash::extract($locationsList, '{n}.Location'));
                ...
                $this->Type->contain();
                $typesList = $this->Type->find('all', ['fields'     => ['id','longname'],
                                                       'conditions' => ['active'=>'1']]);

                $typesList = Hash::sort($typesList, '{n}.Type.longname', 'desc');
                $this->set('typesList', Hash::extract($typesList, '{n}.Type'));
                ...
                $this->GameTemplate->contain(['Type' => ['fields' => 'id']]);

                $gameTemplates = $this->GameTemplate->find('all',
                                                           ['conditions' => ['active' => '1'],
                                                            'fields' => ['id', 'name', 'longname', 'slots_value', 'slots_min', 'slots_max', 'price', 'pricePrivatePassword', 'pricePrivatePower', 'discountPrivatePassword', 'discountPrivatePower']]);

                $gameTemplatesByType = array();

                foreach ($gameTemplates as $gameTemplate)
                {
                    $gameTemplatesByType[$gameTemplate['Type'][0]['id']][] = $gameTemplate['GameTemplate'];
                }
