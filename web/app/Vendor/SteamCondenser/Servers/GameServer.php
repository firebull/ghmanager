...
abstract class GameServer extends Server {
    ...
    protected function handleResponseForRequest($requestType, $repeatOnFailure = true) {
        ...
        if($responsePacket instanceof S2AINFOBasePacket) {
            ...
            // Added by Nikita Bulaev
            // to correctly handle OLD Gold Source Servers info
            $tmp = $responsePacket->getInfo();

            if (!empty($tmp['gameVersion']) and !$this->isOldServer)
            {
                $this->isOldServer = true;
                $this->infoHash = $tmp;
            }
